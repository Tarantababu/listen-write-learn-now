...
const ExercisesPage: React.FC = () => {
  ...
  const directoryExercises = currentDirectoryId ? languageExercises.filter(ex => ex.directoryId === currentDirectoryId) : languageExercises;

  // Then apply additional filters to the directory-filtered exercises
  const filteredExercises = directoryExercises.filter(ex => {
    // Search term filter
    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      return ex.title.toLowerCase().includes(searchLower) || ex.text.toLowerCase().includes(searchLower) || ex.tags.some(tag => tag.toLowerCase().includes(searchLower));
    }
    return true;
  }).filter(ex => {
  ...
  const paginatedExercises = filteredExercises.slice((currentPage - 1) * exercisesPerPage, currentPage * exercisesPerPage);
  ...
  const EnhancedDirectoryBrowser = () => {
    ...
    return <div className={`transition-all duration-300 ease-in-out ${isDirectoryCollapsed ? 'w-12' : 'w-full md:min-w-80'}`}>
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm h-fit">
          ...
          <div className="p-3 md:p-4 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-xl">
            <div className="flex items-center justify-between">
              {!isDirectoryCollapsed && <div className="flex items-center space-x-2">
                  <Folder className="h-5 w-5 text-blue-600" />
                  <h2 className="text-base md:text-lg font-semibold text-gray-900">Folders</h2>
                </div>}
              <Button variant="ghost" size="sm" onClick={() => setIsDirectoryCollapsed(!isDirectoryCollapsed)} className="h-8 w-8 p-0 hover:bg-white/50">
                {isDirectoryCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
              </Button>
            </div>
            
            {!isDirectoryCollapsed && currentDirectoryId && <div className="mt-3 pt-3 border-t border-white/30">
          ...
          {!isDirectoryCollapsed && <div className="p-3 md:p-4">
              {/* Custom Directory Browser UI */}
              <div className="space-y-2">
                {/* All Exercises Option */}
                <div onClick={() => {/* Handle all exercises click - you'll need to implement this based on your DirectoryContext */}} className={`
                    flex items-center justify-between p-2 md:p-3 rounded-lg cursor-pointer transition-all duration-200
                    ${!currentDirectoryId ? 'bg-blue-100 border border-blue-200 shadow-sm' : 'hover:bg-gray-50 border border-transparent'}
                  `}>
                  ...
                  <div className={`
                    ...
                    ${!currentDirectoryId ? 'bg-blue-200 text-blue-800' : 'bg-gray-100 text-gray-600'}
                  `}>
                ...
                <div className="space-y-1">
                  <DirectoryBrowser onExerciseClick={handlePractice} showExercises={true} filterByLanguage={settings.selectedLanguage} />
                </div>
              </div>
              
              {/* Subscription Upgrade Card */}
              {!subscription.isSubscribed && <div className="mt-4 md:mt-6 p-3 md:p-4 bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-lg">
                  <div className="flex items-start space-x-2 md:space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Button size="sm" className="mt-2 md:mt-3 w-full bg-purple-600 hover:bg-purple-700 text-white text-xs" onClick={() => navigate('/dashboard/subscription')}>
                        Upgrade Now
                      </Button>
                    </div>
                  </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30" key={key}>
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="mb-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <div className="flex items-center space-x-4 text-sm text-gray-600">
                  ...
                  {currentDirectoryId && <div className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
                      In selected folder
                    </div>}
                </div>
              </div>
              <Button onClick={() => setIsAddModalOpen(true)} disabled={!canCreateMore} size="lg" className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 disabled:from-gray-400 disabled:to-gray-500">
            ...
            {!subscription.isSubscribed && <Alert className="mt-6 border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50">
                ...
                <AlertDescription className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="w-20 sm:w-24 bg-amber-200 rounded-full h-2">
                        <div className="bg-amber-600 h-2 rounded-full transition-all duration-300" style={{
                      width: `${Math.min(exercises.length / exerciseLimit * 100, 100)}%`
                    }} />
                      </div>
                  ...
                  <Button variant="outline" size="sm" className="w-full sm:w-auto border-amber-300 text-amber-700 hover:bg-amber-100" onClick={() => navigate('/dashboard/subscription')}>
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              {!currentDirectoryId && <div className="mb-8">
              ...
              <div className="mb-6">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                  <FilterBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} selectedTag={selectedTag} setSelectedTag={setSelectedTag} allTags={allTags} />
                </div>
              </div>
              
              {filteredExercises.length === 0 ? <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                  <EmptyStateMessage onCreateExercise={() => canCreateMore ? setIsAddModalOpen(true) : null} />
                </div> : <div className="space-y-6">
                  {/* Enhanced Exercise Grid */}
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <ExerciseGrid paginatedExercises={paginatedExercises} exercisesPerPage={exercisesPerPage} onPractice={handlePractice} onEdit={handleEdit} onDelete={handleDelete} onMove={handleMove} onCreateClick={() => setIsAddModalOpen(true)} canEdit={canEdit} />
                  </div>
        ...
        <ExerciseFormModal isOpen={isEditModalOpen} onOpenChange={handleEditModalClose} initialValues={exerciseToEdit} mode="edit" />
        
        <DeleteExerciseDialog isOpen={isDeleteDialogOpen} onOpenChange={handleDeleteDialogClose} onConfirm={confirmDelete} />
        
        {/* Only render practice modal when there's an exercise to practice */}
        {exerciseToPractice && <PracticeModal isOpen={isPracticeModalOpen} onOpenChange={handlePracticeModalClose} exercise={exerciseToPractice} onComplete={handlePracticeComplete} />}
        
        {/* Move exercise modal */}
        {exerciseToMove && <MoveExerciseModal isOpen={isMoveModalOpen} onOpenChange={handleMoveModalClose} exercise={exerciseToMove} onSuccess={refreshPage} />}
      </div>
    </DirectoryProvider>;
};
