...
// Stripe Price IDs for each plan and currency combination
const STRIPE_PRICE_IDS = {
  monthly: {
    USD: "price_1234567890abcdef_monthly_usd",
    EUR: "price_1234567890abcdef_monthly_eur",
    GBP: "price_1234567890abcdef_monthly_gbp",
    CAD: "price_1234567890abcdef_monthly_cad",
    AUD: "price_1234567890abcdef_monthly_aud",
  },
  quarterly: {
    USD: "price_1234567890abcdef_quarterly_usd",
    EUR: "price_1234567890abcdef_quarterly_eur",
    GBP: "price_1234567890abcdef_quarterly_gbp",
    CAD: "price_1234567890abcdef_quarterly_cad",
    AUD: "price_1234567890abcdef_quarterly_aud",
  },
  annual: {
    USD: "price_1234567890abcdef_annual_usd",
    EUR: "price_1234567890abcdef_annual_eur",
    GBP: "price_1234567890abcdef_annual_gbp",
    CAD: "price_1234567890abcdef_annual_cad",
    AUD: "price_1234567890abcdef_annual_aud",
  },
  lifetime: {
    USD: "price_1234567890abcdef_lifetime_usd",
    EUR: "price_1234567890abcdef_lifetime_eur",
    GBP: "price_1234567890abcdef_lifetime_gbp",
    CAD: "price_1234567890abcdef_lifetime_cad",
    AUD: "price_1234567890abcdef_lifetime_aud",
  },
} as const

// Helper function to get the correct Stripe price_id
const getPriceId = (planId: string, currency: string): string | null => {
  const priceIds = STRIPE_PRICE_IDS[planId as keyof typeof STRIPE_PRICE_IDS]
  if (!priceIds) return null
  return priceIds[currency as keyof typeof priceIds] || null
}
...
const SubscriptionPage: React.FC = () => {
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    try {
      // Get the correct Stripe price_id for the plan and currency
      const priceId = getPriceId(planId, selectedCurrency)
      if (!priceId) {
        toast.error(`Price not available for ${selectedCurrency}. Please select a different currency.`)
        return
      }
...
const PlanCard: React.FC<PlanCardProps> = ({
  ...
  // Check if price is available for the selected currency
  const priceId = getPriceId(plan.id, currency)
  const isPriceAvailable = !!priceId
    ...
    >
      ...
      <CardContent className="flex-grow">
        <div className="space-y-4">
          <div>
            ...
            {!isPriceAvailable && <p className="text-sm text-amber-600 mt-1">Not available in {currency}</p>}
          </div>
      ...
      <CardFooter className="mt-auto">
        <Button
          ...
          disabled={isProcessing || isActive || !isPriceAvailable}
          onClick={() => {
