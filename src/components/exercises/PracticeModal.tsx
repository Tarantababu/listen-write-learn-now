...
const PracticeModal: React.FC<PracticeModalProps> = ({ isOpen, onOpenChange, exercise, onComplete }) => {
  ...
  const getModalStyles = () => {
    ...
    if (isKeyboardOpen && practiceStage === PracticeStage.DICTATION) {
      return "w-[100vw] h-[100vh] max-w-none max-h-none rounded-none m-0 p-0 border-0"
    }
  ...
  return (
    <>
      {/* Inject styles for keyboard handling */}
      {isMobile && (
        <style jsx global>{`
          .keyboard-open {
          ...
          .keyboard-open .dictation-content {
      ...
      <Dialog open={isOpen} onOpenChange={handleOpenChange}>
        <DialogContent
          className={`
          ${getModalStyles()} 
          overflow-hidden flex flex-col ${getContentStyles()}
        `}
        >
          ...
          {practiceStage === PracticeStage.PROMPT && (
            <div className={`${isMobile ? "px-4 py-4" : "px-6 py-8"} space-y-4 md:space-y-6 flex-1 overflow-y-auto`}>
                  ...
                  <CardContent className="p-0">
                    ...
                    >
                      <div className="flex flex-col items-center text-center space-y-2 md:space-y-3">
                        <div
                          className={`flex items-center justify-center bg-primary/10 ${isMobile ? "w-10 h-10" : "w-12 h-12"} rounded-full`}
                        >
                ...
                <Card className="overflow-hidden border border-muted hover:bg-muted/5 transition-all dark:hover:bg-muted/10">
                  <CardContent className="p-0">
                    ...
                    >
                      <div className="flex flex-col items-center text-center space-y-2 md:space-y-3">
                        <div
                          className={`flex items-center justify-center bg-muted/40 ${isMobile ? "w-10 h-10" : "w-12 h-12"} rounded-full`}
                        >
              ...
              {!analysisAllowed && !subscription.isSubscribed && (
                <div
                  className={`bg-amber-50 border border-amber-200 text-amber-800 p-3 md:p-4 rounded-md flex items-start ${isMobile ? "mt-4" : "mt-6"} dark:bg-amber-950/20 dark:border-amber-800/40 dark:text-amber-300`}
                >
                  <AlertTriangle
                    className={`${isMobile ? "h-4 w-4" : "h-5 w-5"} text-amber-600 dark:text-amber-400 mr-2 md:mr-3 flex-shrink-0 mt-0.5`}
                  />
                  <div>
          ...
          {practiceStage === PracticeStage.DICTATION && (
            <div
              className={`flex-1 flex flex-col overflow-hidden ${isMobile && isKeyboardOpen ? "dictation-content" : ""}`}
            >
              <DictationPractice
                exercise={updatedExercise}
