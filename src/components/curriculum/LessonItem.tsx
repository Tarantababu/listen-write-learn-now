...
interface LessonItemProps {
  ...
  onPractice: () => void;
  onAdd: () => void;
  exercise?: Exercise; // Add exercise prop to open modal
}
...
const LessonItem: React.FC<LessonItemProps> = ({
  ...
  const handleModalComplete = (accuracy: number) => {
    setPracticeModalOpen(false);
    // Call the original onPractice callback if needed
    onPractice();
  };
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center ml-2">
                {isCompleted ? (
                  <Button size="sm" variant="outline" className="h-8 gap-1" onClick={handlePracticeClick}>
                    <Play className="h-3.5 w-3.5" />
                    <span className="text-xs">Review</span>
                  </Button>
                ) : isInProgress ? (
                  <Button size="sm" variant="default" className="h-8 gap-1" onClick={handlePracticeClick}>
                    <Play className="h-3.5 w-3.5" />
