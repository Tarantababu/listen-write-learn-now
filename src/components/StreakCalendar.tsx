        ...
        <div className="px-2">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-7 gap-1 mb-2">
              {weekDays.map(day => <div key={day} className="text-center text-xs font-medium text-muted-foreground h-8 flex items-center justify-center">
                  {day}
                </div>)}
            </div>
            ...
            <div className="grid grid-cols-7 gap-1">
              {monthDays.map(date => {
              const activity = getActivityForDate(date);
              return <div key={date.toISOString()} className={cn(getDayClassName(date))} title={activity?.hasActivity ? `${activity.exerciseCount} exercises completed` : 'No activity'}>
                    {format(date, 'd')}
                    {activity?.hasActivity && <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-primary-foreground rounded-full" />}
                  </div>;
            })}
            </div>
          </div>
          
          {loading && <div className="text-center text-muted-foreground mt-4">
