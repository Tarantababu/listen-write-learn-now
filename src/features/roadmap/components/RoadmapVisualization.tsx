...
const RoadmapNodeComponent: React.FC<RoadmapNodeProps> = ({ 
  node, 
  status, 
  progress = 0, 
  onNodeClick 
...
const RoadmapVisualization: React.FC<RoadmapVisualizationProps> = ({ onNodeSelect }) => {
  ...
  if (nodes.length === 0) {
    return (
      <div className="text-center p-4">
        <p className="text-muted-foreground">No roadmap selected or empty roadmap</p>
      </div>
    );
  }
  ...
  const getNodeStatus = (node: RoadmapNode): 'completed' | 'current' | 'locked' | 'available' => {
    ...
    if (isCurrentNode(node.id)) return 'current';
    if (isNodeAccessible(node.id)) return 'available';
    return 'locked';
  };
