...
serve(async (req) => {
  ...
  try {
    ...
    if (isCustomText && customText) {
      content = await processCustomTextEnhanced(customText, language, difficulty_level, grammar_focus, timeoutController.signal);
    } else {
...
async function processCustomTextEnhanced(
  customText: string, 
  language: string, 
  difficulty_level: string, 
  grammar_focus?: string,
  ...
  console.log(`[CUSTOM ENHANCED] Processing ${customText.length} characters with enhanced analysis`);
  
  const prompt = `Analyze this custom text for ${difficulty_level} level ${language} learners with enhanced pedagogical insights.

Text: "${customText}"
${grammar_focus ? `Grammar focus: ${grammar_focus}` : ''}

ENHANCED CUSTOM TEXT REQUIREMENTS:
1. Break into natural, meaningful sentences
2. Provide comprehensive word analysis (limit to 8 key words per sentence)
3. Preserve original meaning while adding educational value
4. Include contextual usage explanations
5. Assess complexity and learning objectives
...
Return ONLY this JSON with enhanced analysis:
{
        ...
        "words": [
          {
            ...
            "partOfSpeech": "noun/verb/adjective/etc",
            "difficulty": "easy/medium/hard",
            "contextualUsage": "how it's used in this specific context"
          }
        ],
        "grammar": ["specific grammar points demonstrated"],
        "translation": "accurate English translation",
  ...
  "analysis": {
    ...
    "customTextAnalysis": {
      ...
      "processingMethod": "enhanced_custom",
      "preservedMeaning": true,
      "educationalEnhancements": ["word analysis", "grammar focus", "cultural context"]
    },
    "learningObjectives": ["what students will gain from this text"],
    "culturalContext": "relevant cultural information if applicable"
  }
}`;

  return await callOpenAIEnhanced(prompt, 4000, 30000, signal, 'custom_enhanced');
}
